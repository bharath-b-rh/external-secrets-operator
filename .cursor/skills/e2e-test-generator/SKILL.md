...
**Output directory**: `output/e2e_<repo-name>/` (e.g., `output/e2e_external-secrets-operator/`). The `<repo-name>` is derived from the Go module path basename. With `--output <path>`, use `<path>/e2e_<repo-name>/`. Create the directory if it does not exist.
