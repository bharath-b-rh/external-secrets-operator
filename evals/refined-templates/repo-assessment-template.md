...
### 4.6 Trusted CA / TLS Trust Baseline (OpenShift operators)
* Document **proxy-gated** platform CA injection if present (ConfigMap name, inject label, mount path, gate function).
* Distinguish **operator-owned** trust config vs **upstream operand** store/provider TLS fields (e.g., `caProvider`).
* List all CRD types embedding shared config structs (e.g., `CommonConfigs`) — codegen may update multiple CRDs.
* Note Kubernetes limitation: ConfigMap volumes are **same-namespace only**; cross-namespace refs require sync or read-via-API patterns.
