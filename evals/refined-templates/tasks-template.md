...
- **API field tasks:** when adding CRD fields, pair `make test-apis` with a **manifests/codegen task**
  that runs before or with API verification — envtest requires current CRD schema (`unknown field` otherwise).
