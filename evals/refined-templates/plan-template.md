...
**Trusted CA / enterprise PKI features (operators):** resolve and document:
- Proxy-gated platform CA vs enterprise CA independence (decision matrix, e.g., A-011).
- Cross-namespace ConfigMap ref mount strategy (sync to operand namespace vs same-ns only).
- Which deployment assets receive mounts (controller/webhook only vs all operands).
- Operator ESC field vs upstream store `caProvider` — independent paths (no auto-sync).
