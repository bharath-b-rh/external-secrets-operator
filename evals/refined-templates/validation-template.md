...
**Operator CA / trust features:** verify spec distinguishes (1) operator ESC/operand pod trust,
(2) upstream store `caProvider` config-only paths, (3) proxy-gated platform CA vs enterprise CA
independence. Flag if spec requires upstream CRD/bindata changes when repo assessment shows fields already exist.
